...
class BaseEventHandler(tcod.event.EventDispatch[ActionOrHandler]):
    def handle_events(self, event: tcod.event.Event) -> BaseEventHandler:
        """Handle an event and return the next axctive event handler."""
...
class EventHandler(BaseEventHandler):
    ...
    def handle_events(self, event: tcod.event.Event) -> BaseEventHandler:
        """Handle events for unput handlers with an engine"""
        ...
        if self.handle_action(action_or_state):
            ...
            if not self.engine.player.is_alive:
                # The player was killed sometimes during or after the action.
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> MainGameEventHandler | None:
        ...
        if event.sym in CURSOR_Y_KEYS:
            ...
            if adjust < 0 and self.cursor == 0:
                # Only move from top to the bottom when you're on the edge.
                self.cursor = self.log_length - 1
            elif adjust > 0 and self.cursor == self.cursor == self.log_length - 1:
...
class InventoryEventHandler(AskUserEventHandler):
    """This handle lets the user select an item.
...
class SelectIndexHandler(AskUserEventHandler):
    ...
    def set_cursor_pos(
        self, mods_pressed: tcod.event.Modifier, key: tcod.event.KeySym
    ) -> None:
        modifier = 1  # Holding modifier keys will speed up key movement.
        if mods_pressed & (tcod.event.KMOD_LSHIFT | tcod.event.KMOD_RSHIFT):
            modifier *= 5
        if mods_pressed & (tcod.event.KMOD_LCTRL | tcod.event.KMOD_RCTRL):
            modifier *= 10
        if mods_pressed & (tcod.event.KMOD_LALT | tcod.event.KMOD_RALT):
            modifier *= 20
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> ActionOrHandler | None:
        ...
        key = event.sym
        if key in MOVE_KEYS:
            mods_pressed = event.mod
            self.set_cursor_pos(mods_pressed, key)
            return None
